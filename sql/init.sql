...
-- Tabela de status genéricos (usado tanto para solicitações quanto projetos)
CREATE TABLE IF NOT EXISTS tb_item_status (
    idStatus INT AUTO_INCREMENT PRIMARY KEY,
    nmStatus VARCHAR(100) NOT NULL,
    dsStatus VARCHAR(255),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Tabela de status de solicitações (mantida por compatibilidade)
...
CREATE TABLE IF NOT EXISTS tb_projeto (
    ...
    FOREIGN KEY (idStatusProjeto) REFERENCES tb_item_status(idStatus),
...
-- Inserir status genéricos (tb_item_status)
INSERT IGNORE INTO tb_item_status (idStatus, nmStatus, dsStatus) VALUES 
(1, 'Pendente', 'Aguardando análise ou início'),
(2, 'Em Análise', 'Em processo de avaliação'),
(3, 'Aprovado', 'Aprovado para execução'),
(4, 'Rejeitado', 'Não aprovado'),
(5, 'Em Andamento', 'Em execução'),
(6, 'Concluído', 'Finalizado com sucesso'),
(7, 'Pausado', 'Temporariamente suspenso'),
(8, 'Cancelado', 'Cancelado pelo solicitante ou administrador');

-- Inserir status de solicitações (mantido por compatibilidade)
